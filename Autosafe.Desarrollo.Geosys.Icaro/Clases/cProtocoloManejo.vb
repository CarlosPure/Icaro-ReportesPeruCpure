...
Public Class cProtocoloManejo : Inherits cGeneracion

    Private dsProtocolo As DataSet
    Private Const Reporte As String = "Reporte de Protocolo Manejo de la Flota"
    ...
    Public Overrides Sub Generar(Datos As Object)
        Console.WriteLine(Datos.usuarioId.ToString() & ": Obteniendo Datos para el Reporte Protocolo Manejo")
    ...
    Private Function GenerarReporte(ByVal IdUsuario As Integer,
        ...
        Try
            Hoja.Header.Center = "Reporte Protocolo Manejo Flota"
            ...
            Console.WriteLine(String.Format("Obteniendo Protocolo Manejo de la Flota: {0} {1}", Fecha2ANSI(Me.FechaInicial.ToString(), True), Fecha2ANSI(Me.FechaFinal.ToString(), True)))
            '******************************************************************************
            ...
            'AGREGAR LA HORA DE INICIO Y FIN A LA CONSULTA DEL REPORTE
            '******************************************************************************

            Dim sql As String

            If (My.Settings.Pais = "PE") Then
                sql = String.Format("[sp_driving_protocol] '{0}','{1}','{2}','{3}'",
                           IdUsuario,
                           Me.IdEntidad,
                            Fecha2ANSI(FechaInicial.ToString(), True) & " " & HoraInicio,
                            Fecha2ANSI(FechaFinal.ToString(), True) & " " & HoraFin
                           )
            Else
                sql = String.Format("[sp_driving_protocol] '{0}','{1}','{2}','{3}'",
                           IdUsuario,
            ...
            '******************************************************************************
            'TODO CAMBIOS REALIZADOS POR VICTOR.VEGA 20191212
            ...
            'Else
            '    Dim vnxls As New fGrid(dsProtocolo, Me.Archivo, CDbl(Parametros))
            '    vnxls.GenerarArchivo()
            'End If

            Dim vnxls As New fProtocoloManejoGrid(dsProtocolo, Me.Archivo, CDbl(0))
            vnxls.GenerarArchivo()
        ...
        Finally
            Try
                ...
                End If
            Catch ex As Exception
                Console.WriteLine("No se puedo cerrar la conexion")
            Finally
